...
def lora_merge(w,lora,lora_alpha,device="cuda",layer_filter=None,):
...
class RWKV(MyModule):
    def __init__(self, model, strategy, verbose = True, convert_and_save_and_exit = None, lora=None,lora_alpha=0,lora_layer_filter=None):
        ...
        with torch.no_grad():
            if lora:
                self.w: Dict[str, torch.Tensor] = torch.load(args.MODEL_NAME, map_location='cpu')
                # self.w = lora_merge(w=self.w,lora='/root/autodl-tmp/patch.pth',
                #      lora_alpha=lora_alpha,layer_filter='-1.5*5',
                #     device=('cuda' if 'cuda' in strategy else 'cpu'))
                self.w = lora_merge(w=self.w,lora=lora,
